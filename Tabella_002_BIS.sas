...
*tengo solo l'ULTIMA VALUTAZIONE A PRESCINDERE DAL TIPO;

PROC SORT DATA=VALUTAZIONI;
	BY codice_soggetto_bin DESCENDING DATA_VALUTAZIONE;
RUN;
...
data NO_DUPL_V4;
	set NO_DUPL_V4;
	key	= cats(ULSS_new, CODICE_SOGGETTO_BIN, ID_EPISODIO);
run;
/****** ++++ ulss*/
data ana ; set ana; 
		if ULSS='101' THEN ULSS_NEW=501;
	if ULSS='102' THEN ULSS_NEW=501;
	if ULSS='109' THEN ULSS_NEW=502;
	if ULSS='107' THEN ULSS_NEW=502;
	if ULSS='108' THEN ULSS_NEW=502;
	if ULSS='112' THEN ULSS_NEW=503;
	if ULSS='113' THEN ULSS_NEW=503;
	if ULSS='114' THEN ULSS_NEW=503;
	if ULSS='110' THEN ULSS_NEW=504;
	if ULSS='118' THEN ULSS_NEW=505;
	if ULSS='119' THEN ULSS_NEW=505;
	if ULSS='115' THEN ULSS_NEW=506;
	if ULSS='116' THEN ULSS_NEW=506;
	if ULSS='117' THEN ULSS_NEW=506;
	if ULSS='103' THEN ULSS_NEW=507;
	if ULSS='104' THEN ULSS_NEW=507;
	if ULSS='106' THEN ULSS_NEW=508;
	if ULSS='105' THEN ULSS_NEW=508;
	if ULSS='120' THEN ULSS_NEW=509;
	if ULSS='121' THEN ULSS_NEW=509;
	if ULSS='122' THEN ULSS_NEW=509;
	if ULSS='501' THEN ULSS_NEW=501;
	if ULSS='502' THEN ULSS_NEW=502;
	if ULSS='503' THEN ULSS_NEW=503;
	if ULSS='504' THEN ULSS_NEW=504;
	if ULSS='505' THEN ULSS_NEW=505;
	if ULSS='506' THEN ULSS_NEW=506;
	if ULSS='507' THEN ULSS_NEW=507;
	if ULSS='508' THEN ULSS_NEW=508;
	if ULSS='509' THEN ULSS_NEW=509;
	run; 
...
DATA ARCIPELAGO2_BIS;
	MERGE V5 (IN=A) au_COM (IN=B);
	BY CODICE_SOGGETTO_BIN;
	IF A;
RUN;

/*TENGO LE VARIABILI PER LA TABELLA 2 BIS*/
data ARCIPELAGO2_BIS;
	set ARCIPELAGO2_BIS;
	keep 
	tipo_valutazione
...
proc summary data=ARCIPELAGO2_BIS ;
	class ULSS_a2;
...
PROC SORT DATA=ARCIPELAGO2_BIS;
	BY ULSS_A2;
RUN;/*27155*/
...
data arcipelago3;
	MERGE ARCIPELAGO2_BIS (in=a) Y (in=b);
	BY ULSS_A2;
...
DATA ARCIPELAGO2_bis_OK;
	SET arcipelago33;
RUN;

data ARCIPELAGO2_bis_OK; set ARCIPELAGO2_bis_OK; drop _freq_ _type_ anno; run; 

data ARCIPELAGO2_bis_OK; set arcipelago2_bis_ok;
keep 
		ID_VALUTAZIONE
		CODICE_SOGGETTO_BIN
		ULSS_A2
...
DATA TMP_&anno.; SET ARCIPELAGO2_bis_OK; RUN; 
/*solo per aggiunta fasi in più*/
DATA TMP1_&anno.; SET ARCIPELAGO2_bis_OK; RUN; 


DATA ARCIPELAGO_002_BIS; SET TMP_2016 TMP_2017 TMP_2018 TMP1_2018; RUN; 

data arcipelago_002_bis; set arcipelago_002_bis;
...
run;
/*cancello le ulss old e lascio solo le ulss new dal 2017 in poi*/
data arcipelago_002_bis; set arcipelago_002_bis;
if anno_a2>=2017 then ulss_a2='000'; 
if codice_soggetto_bin='004EB381469CE373' and anno_val=2016 then ulss_a2='101';  
if codice_soggetto_bin='00414E6ADD1A68E9' and anno_val=2017 then ulss_a2='501'; 
if ulss_udo=. then ulss_udo ='000';
run; 

/*+++++++++++++*/
/*aggiungo variabile ULSS_UDO_NEW*/
data arcipelago_002_bis; 
set arcipelago_002_bis;
	...
	if ULSS_UDO= '509'  then ULSS_UDO_NEW= 509 ;
		if ULSS_UDO= '000'  then ULSS_UDO_NEW= 999 ;
				if ULSS_UDO= '999'  then ULSS_UDO_NEW= 999 ;
run; 

/*PER IL MOMENTO METTO ZERO SE NON CALCOLO GIORNI ATTESA E PUNTEGGIO SVAMA */

/*proc freq data=arcipelago_002_bis; table ulss_a2 ulss_new ulss_udo ulss_udo_new; run; */
data arcipelago_002_bis; 
set arcipelago_002_bis;
...
/*salvo le tabelle per anno per evitare il timeout di arcipelago*/
%let today=%sysfunc(today(),yymmddn8.);
%put &today.;

data TAB_002_bis_2016_&today.; set arcipelago_002_bis; where anno_a2=2016; run; 
data TAB_002_bis_2017_&today.; set arcipelago_002_bis; where anno_a2=2017; run; 
data TAB_002_bis_2018_&today.; set arcipelago_002_bis; where anno_a2=2018; run; 
